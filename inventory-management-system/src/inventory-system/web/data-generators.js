// Dynamic Data Generators for Frontend
// JavaScript version of the TypeScript data generators
...
function generateComponentId(componentType) {
...
function generateSupplierQuotes(componentType, urgency = 'normal') {
  const spec = getComponentSpec(componentType);
  const quotes = [];
  ...
  for (const supplier of selectedSuppliers) {
    ...
    if (availability) {
      quotes.push({
        ...
        days: deliveryDays,
        currency: 'USD',
        rush: supplier.deliverySpeed > 0.7 && urgency === 'urgent',
  ...
  quotes.sort((a, b) => {
    const scoreA = a.price + (a.days * 50);
    const scoreB = b.price + (b.days * 50);
...
function generateInstallationInstructions(componentType) {
  const spec = getComponentSpec(componentType);
  const instructions = [];
